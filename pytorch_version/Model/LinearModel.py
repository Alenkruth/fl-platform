...
class LinearModel:
    ...
    def ComputeLoss(self, para, x , y): 
        ...
        y_pre = x @  para - y
        # Access l1_lambda without invoking FLAGS.__getattr__ (which emits a RuntimeWarning
        # if flags haven't been parsed). We read the internal dict safely and fall back to 0.
        try:
            import gflags as _gflags
            _flags_obj = _gflags.FLAGS
            _flags_dict = getattr(_flags_obj, '__dict__', None)
            l1 = _flags_dict.get('l1_lambda', 0) if _flags_dict is not None else 0
        except Exception:
            l1 = 0
        return (y_pre.T @ y_pre)[0][0] / np.shape(x)[0] + np.linalg.norm( para, 1) * l1
